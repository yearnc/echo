...
**当前代码规模**：`lib/` 约 35 个文件，含 core/domain/data/features 四层。

### 8.2 2026-09-11 夜 · 实测反馈与修复

项目作者拿着真机（桌面预览）试玩后提了四条，逐条修复：
...
**已验证的运行时事实（含本轮）**：发帖后 12 条排期入库（8 赞 + 4 评）；冷启动补发逐条兑现 11 条待办；前台心跳独立验证通过（待办 3 条全部 `done`，通知 7 → 13，帖子计数 986 → 998）；项目作者实测帖从「1 赞 0 评」涨到「44 赞 4 评」；**51 个测试全过**，`flutter analyze` 零问题。

**待办（明天）**：数据库重置（旧播种内容里还有重复住民）、项目作者重写人设与三幕脚本（文档已放桌面 `ovo`）、M4 清醒模式状态机。
